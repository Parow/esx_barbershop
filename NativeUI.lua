...
UIMenuHorizontalOneLineGridPanel.__call = function() return "UIMenuPanel", "UIMenuHorizontalOneLineGridPanel" end
...
--[[
    ...
    Elements
--]]
--Sprite.New(TxtDictionary, TxtName, 0 + X, 0 + Y, 431, 107,255,255,0,200),
function Sprite.New(TxtDictionary, TxtName, X, Y, Width, Height, Heading, R, G, B, A)
    local _Sprite = {
        ...
        _Colour = {R = tonumber(R) or 255, G = tonumber(G) or 255, B = tonumber(B) or 255, A = tonumber(A) or 220},
    }
...
function UIMenuItem.New(Text, Description, _)
    
    _UIMenuItem = {
        Rectangle = UIResRectangle.New(0, 0, 431, 39, 255, 255, 255, 20),
...
function UIMenu:EditTitle(Caption)
    self.Title:Text(Caption)
end
function UIMenu:SubEditTitle(Caption)
    self.Subtitle.Text:Text(Caption)
end
...
function UIMenuCheckboxItem.New(Text, Check, Description)
    local _UIMenuCheckboxItem = {
        ...
        CheckboxEvent = function(_, _, _) end,
    }
...
function UIMenuListItem.New(Text, Items, Index, Description,price)
    ...
    local _UIMenuListItem = {
        ...
        Label = {
            Text = UIResText.New(price, 0, 0, 0.30, 245, 245, 245, 255, 0, "Right"),
            MainColour = {R = 255, G = 255, B = 255, A = 255},
            HighlightColour = {R = 0, G = 0, B = 0, A = 255},
        },
        _Index = tonumber(Index) or 1,
        Panels = {},
        OnListChanged = function(_, _, _) end,
        OnListSelected = function(_, _, _) end,
    }
    --()
...
function UIMenuListItem:RightLabel(Text, MainColour, HighlightColour)
...
---SetParentItem
---@param Item table
function UIMenuHorizontalOneLineGridPanel:SetParentItem(Item) -- required
...
function UIMenuSliderItem.New(Text, Items, Index, Description, Divider)
    ...
    local _UIMenuSliderItem = {
        ...
        _Index = tonumber(Index) or 1,
        OnSliderChanged = function(_, _, _) end,
        OnSliderSelected = function(_, _, _) end,
    }
...
--   --(Counter)
    ...
    local _UIMenuProgressItem = {
        ...
        OnProgressChanged = function(_, _, _) end,
        OnProgressSelected = function(_, _, _) end,
    }
...
function UIMenuHeritageWindow.New(Mum, Dad)
    ...
    _UIMenuHeritageWindow = {
         -- Background is required, must be a sprite or a rectangle.
...
_UIMenuColourPanel = {
    ...
    Text = UIResText.New(Title.." (1 sur "..#Colours..")" or "Title".." (1 sur "..#Colours..")", 0, 0, 0.35, 255, 255, 255, 255, 0, "Centre"),
    ParentItem = nil,
}
...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName,mouse,R,G,B,A)
    local _UIMenu = nil
    local Txt = nil
    local mousex = false
    if TxtDictionary == "no" then
        Txt = "no"
    end
   -- print(mouse)
    if mouse ~= true then
    ...
    if Subtitle ~= nil then Subtitle = tostring(string.upper(Subtitle)) or "" else Subtitle = "" end
    ...
    if Txt ~= "no" then 
        _UIMenu = {
            Logo = Sprite.New(TxtDictionary, TxtName, 0 + X, 0 + Y, 431, 107,0,R,G,B,A),
            Banner = nil,
            ...
            PageCounter = {PreText = "",isCustom=false},
            Extra = {},
            Description = {},
            Items = {},
            Windows = {},
            ...
            OnIndexChange = function(_, _) end,
            OnListChange = function(_, _, _) end,
            OnSliderChange = function(_, _, _) end,
            OnProgressChange = function(_, _, _) end,
            OnCheckboxChange = function(_, _, _, _) end,
            OnListSelect = function(_, _, _) end,
            OnSliderSelect = function(_, _, _) end,
            OnProgressSelect = function(_, _, _) end,
            OnItemSelect = function(_, _, _) end,
            ...
            Settings = {
                ...
                MouseControlsEnabled = mousex,
                MouseEdgeEnabled = mousex,
                ControlDisablingEnabled = mousex,
                Audio = {
        ...
        _UIMenu = {
            Logo = Sprite.New(TxtDictionary, TxtName, 0 + X, 0 + Y, 0, 107),
            Banner = nil,
            Title = UIResText.New(Title, 215 + X, 20 + Y, 1.15, 255, 255, 255, 255, 1, 1),
            Subtitle = {ExtraY = 0},
            WidthOffset = 0,
    ...
    if Subtitle ~= "" and Subtitle ~= nil then
        ...
        _UIMenu.PageCounter.Text = UIResText.New("eee", 425 + _UIMenu.Position.X, 110 + _UIMenu.Position.Y, 0.35, 245, 245, 245, 255, 0, "Right")
...
function UIMenu:SetBannerColor(R,G,B,A)


    k = Sprite.New("commonmenu", "interaction_bgd", 0 + self.Position.X, 0 + self.Position.Y, 431, 107,0,R,G,B,A)

    self:SetBannerSprite(k)
end
...
function UIMenu:Draw()
    ...
    if self.Items[CurrentSelection]:Description() ~= ""or nil then
    ...
    if #self.Items <= self.Pagination.Total + 1 then
       ...
       -- print(self.PageCounter.isCustom)
...
function UIMenu:ProcessMouse()
    ...
    if IsMouseInBounds(0, 0, 30, 1080) and self.Settings.MouseEdgeEnabled then
        SetGameplayCamRelativeHeading(GetGameplayCamRelativeHeading() + 1)
        SetCursorSprite(6)
...
function UIMenu:AddInstructionButton(button)
    if type(button) == "table" and #button == 2 then
      ...
      --  --(json.encode(self.InstructionalButtons))
    end
end
...
function UIMenuItem:UpdateText(Text)
    print(Text)
    if tostring(Text) and Text ~= nil then
        print(Text)
        self.Text:Text(Text)
...
function UIMenu:UpdateScaleform()
   ...
   PushScaleformMovieFunctionParameterString(GetControlInstructionalButton(2, 176, 0))
    PushScaleformMovieFunctionParameterString("Sélectionner")
    PopScaleformMovieFunction()

    PushScaleformMovieFunction(self.InstructionalScaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(1)
    PushScaleformMovieFunctionParameterString(GetControlInstructionalButton(2, 177, 0))
    PushScaleformMovieFunctionParameterString("Retour")
    PopScaleformMovieFunction()
...
function MenuPool:AddSubMenu(Menu, Text, Description, KeepPosition, KeepBanner,mouse)
    ...
    if Menu() == "UIMenu" then
        ...
        if KeepPosition then
            SubMenu = UIMenu.New(Menu.Title:Text(), Text, Menu.Position.X, Menu.Position.Y,"","",mouse)
            
        else
            
            SubMenu = UIMenu.New(Menu.Title:Text(), Text, 0,0,"","",mouse)
        end
        if KeepBanner then
...
function NativeUI.CreateMenu(Title, Subtitle, X, Y, TxtDictionary, TxtName,mouse,R,G,B,A)
    
    return UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName,mouse,R,G,B,A)
end
...
function UIMenuCheckboxItem:CurrentSelection()
    if tonumber(value) then
        if #self.Data.Items == 0 then
            self.Data.Index = 0
        end
